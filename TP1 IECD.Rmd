...
``````
Podemos observar que no cambia mucho el gráfico de densidad con el uso de distintos núcleos, por lo que quizás el ancho de la ventana sea el parámetro más importante (En este caso R lo está eligiendo automáticamente).
...
```{r}
bagplot(datosBebes$Perim_cef, datosBebes$Talla, approx.limit = 100, 
        xlab = "Perímetro Encefálico", ylab = "Talla", xlim = c(0, 50),
        ylim= c(30,140), main = "Bagplot Perimetro cefálico vs Talla sin Datos atípicos",
        show.outlier = F)
```

NO hay datos atipicos porque los saque daahhh !! ?? preguntar 
...
par(mfrow = c(1, 2))
bagplot(datos_mujeres$Perim_cef, datos_mujeres$Talla,ylim=c(20,120),main="Datos mujeres")
grid()
bagplot(datos_hombres$Perim_cef, datos_hombres$Talla,ylim=c(20,120),main="Datos varones")
grid()
```
